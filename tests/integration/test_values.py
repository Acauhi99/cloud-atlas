...
async def test_get_value(client: AsyncClient, user_id: uuid.UUID):
    ...
    resp = await client.get(
        f"/tags/{tag_id}/values/{val_id}", headers={"X-User-ID": str(user_id)}
    )
    assert resp.status_code == 200
...
async def test_value_nonexistent_tag_returns_404(
    ...
    resp = await client.get(
        f"/tags/{fake_id}/values", headers={"X-User-ID": str(user_id)}
    )
...
async def test_value_cross_user_access_denied(client: AsyncClient):
    ...
    uid2 = str(uuid.uuid4())
    tag_resp = await client.post(
        "/tags", json={"name": "cross-tag"}, headers={"X-User-ID": uid1}
    )
    tag_id = tag_resp.json()["id"]
    val_resp = await client.post(
        f"/tags/{tag_id}/values",
...
async def test_duplicate_value_name_in_same_tag_returns_409(
    ...
    tag_resp = await client.post(
        "/tags", json={"name": "dup-val-parent"}, headers={"X-User-ID": str(user_id)}
    )
    tag_id = tag_resp.json()["id"]
    await client.post(
        f"/tags/{tag_id}/values",
...
async def test_same_value_name_in_different_tags_allowed(
    ...
    tag1 = await client.post(
        "/tags", json={"name": "val-parent-1"}, headers={"X-User-ID": str(user_id)}
    )
    tag2 = await client.post(
        "/tags", json={"name": "val-parent-2"}, headers={"X-User-ID": str(user_id)}
    )
    resp1 = await client.post(
        f"/tags/{tag1.json()['id']}/values",
        json={"name": "shared-val"},
        headers={"X-User-ID": str(user_id)},
    )
    resp2 = await client.post(
        f"/tags/{tag2.json()['id']}/values",
        json={"name": "shared-val"},
        headers={"X-User-ID": str(user_id)},
    )
...
async def test_invalid_value_name(
    ...
    tag_resp = await client.post(
        "/tags", json={"name": "valvalid-parent"}, headers={"X-User-ID": str(user_id)}
    )
    tag_id = tag_resp.json()["id"]
    resp = await client.post(
        f"/tags/{tag_id}/values",
...
async def test_value_description_omitted_is_null(
    ...
    tag_resp = await client.post(
        "/tags", json={"name": "vno-desc"}, headers={"X-User-ID": str(user_id)}
    )
    tag_id = tag_resp.json()["id"]
    resp = await client.post(
        f"/tags/{tag_id}/values",
...
async def test_user_cannot_update_other_users_value(client: AsyncClient):
    ...
    uid2 = str(uuid.uuid4())
    tag_resp = await client.post(
        "/tags", json={"name": "owner-vtag"}, headers={"X-User-ID": uid1}
    )
    tag_id = tag_resp.json()["id"]
    val_resp = await client.post(
        f"/tags/{tag_id}/values",
...
async def test_user_cannot_delete_other_users_value(client: AsyncClient):
    ...
    uid2 = str(uuid.uuid4())
    tag_resp = await client.post(
        "/tags", json={"name": "del-vtag"}, headers={"X-User-ID": uid1}
    )
    tag_id = tag_resp.json()["id"]
    val_resp = await client.post(
        f"/tags/{tag_id}/values",
